...
function RecipeDetailPage({ shoppingListItems, onAddToShoppingList, savedPreferences, onPreferencesChange }) {
  const { id } = useParams();
  ...
  useEffect(() => {
    ...
    // Sync with shopping list items whenever they change
    const checkedFromList = new Set();
    if (shoppingListItems) {
      shoppingListItems.forEach(item => {
        if (item.recipeId === recipe.id && item.userHas === true) {
      ...
      if (checkedFromList.size > 0) {
        setCheckedIngredients(checkedFromList);
      }
    }
  }, [shoppingListItems, recipe]);
  ...
  const handleIngredientToggle = ingredientId => {
    setCheckedIngredients(prev => {
      ...
      // Save preferences so they persist when page reopens
